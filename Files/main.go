...
func main() {
    ...
    // آپدیت configs برای پشتیبانی از پروتکل‌های جدید
    ...
    // آپدیت myregex به فرمت جدید
    myregex := map[string][]string{
        "Vmess":        []string{`vmess://[^ \n\r<\"']+`},
        "Vless":        []string{`vless://[^ \n\r<\"']+`},
        "Trojan":       []string{`trojan://[^ \n\r<\"']+`},
        "ShadowSocks":  []string{`ss://[^ \n\r<\"']+`, `shadowsocks://[^ \n\r<\"']+`},
        "ShadowSocksR": []string{`ssr://[^ \n\r<\"']+`},
        "Tuic":         []string{`tuic://[^ \n\r<\"']+`, `tuic5://[^ \n\r<\"']+`},
        "Hysteria2":    []string{`hy2://[^ \n\r<\"']+`, `hysteria2://[^ \n\r<\"']+`},
        ...
        if all_messages {
            ...
            doc.Find(".tgme_widget_message_text").Each(func(j int, s *goquery.Selection) {
                ...
                for a := 0; a < len(lines); a++ {
                    for proto, regex_values := range myregex {
                        // بررسی همه regexهای مربوط به یک پروتکل
                        for _, regex_value := range regex_values {
                            re := regexp.MustCompile(regex_value)
                            lines[a] = re.ReplaceAllStringFunc(lines[a], func(match string) string {
                                return "\n" + match
                            })
                        }
                        // اضافه کردن به mixed اگر شامل نام پروتکل باشه
                        if strings.Contains(strings.ToLower(lines[a]), strings.ToLower(proto)) {
                            configs["mixed"] += "\n" + lines[a] + "\n"
                        }
                    }
                }
            ...
            doc.Find("code,pre").Each(func(j int, s *goquery.Selection) {
                ...
                lines := strings.Split(message_text, "\n")
                for a := 0; a < len(lines); a++ {
                    for proto_regex, regex_values := range myregex {
                        // بررسی همه regexهای مربوط به یک پروتکل
                        for _, regex_value := range regex_values {
                            re := regexp.MustCompile(regex_value)
                            lines[a] = re.ReplaceAllStringFunc(lines[a], func(match string) string {

...
def output(list, num):
    ...
    # آپدیت برای پشتیبانی از پروتکل‌های جدید
    outputs = {
        'vmess': [],
        'vless': [],
        'trojan': [],
    ...
    for output in modified_output_list:
        if output.startswith("vmess://"):
            outputs['vmess'].append(output)
        elif output.startswith("vless://"):
            outputs['vless'].append(output)
        elif output.startswith("trojan://"):
            outputs['trojan'].append(output)
        elif output.startswith("ssr://"):
            outputs['ssr'].append(output)
        elif output.startswith("ss://") or output.startswith("shadowsocks://"):
            outputs['ss'].append(output)
        elif output.startswith("wireguard://"):
            outputs['wireguard'].append(output)
        elif output.startswith("hy2://") or output.startswith("hysteria2://"):
            outputs['hysteria2'].append(output)
        elif output.startswith("tuic://") or output.startswith("tuic5://"):
            outputs['tuic'].append(output)

    # ذخیره فایل‌های جداگانه برای هر پروتکل
    for protocol, configs in outputs.items():
        with open(os.path.join(splitted_output, f"{protocol}.txt"), 'w') as f:
            f.write("\n".join(configs))
            f.close()
